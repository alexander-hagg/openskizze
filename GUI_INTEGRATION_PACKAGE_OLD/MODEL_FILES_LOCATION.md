...
models/
├── svgp_27m.pth                   (~50 MB)
├── svgp_27m_normalization.json    (~2 KB)
├── unet_27m.pth                   (~50 MB)
└── unet_27m_normalization.json    (~5 KB)
```
...
# Test SVGP
checkpoint = torch.load('models/svgp_27m.pth', weights_only=False)
print(f"SVGP inducing points: {checkpoint['num_inducing']}")
print(f"SVGP input dim: {checkpoint['input_dim']}")

with open('models/svgp_27m_normalization.json') as f:
    svgp_norm = json.load(f)
print(f"SVGP train_y_mean: {svgp_norm['train_y_mean']:.2f}")

# Test U-Net
checkpoint = torch.load('models/unet_27m.pth', weights_only=False)
print(f"U-Net config: {checkpoint['config']}")

with open('models/unet_27m_normalization.json') as f:
