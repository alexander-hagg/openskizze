...
evaluator = create_evaluator('unet', parcel_size=27, device='cuda')
...
evaluator = create_evaluator(
    ...
    parcel_size=27,           # Currently only 27m supported
...
```
models/
├── svgp_27m.pth                   # SVGP model (~50 MB)
├── svgp_27m_normalization.json    # Input/output stats
├── unet_27m.pth                   # U-Net model (~50 MB)
└── unet_27m_normalization.json    # Input/output stats
```
...
python -c "from openskizze_optimization.model_evaluator import create_evaluator; \
           evaluator = create_evaluator('unet', 27); print('✓ OK')"

# Test evaluation speed
python -c "import numpy as np; from openskizze_optimization.model_evaluator import create_evaluator; \
           evaluator = create_evaluator('unet', 27); genomes = np.random.randn(100, 60); \
           sizes = np.full(100, 27.0); evaluator.evaluate(genomes, sizes); print('✓ OK')"
```
