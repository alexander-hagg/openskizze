...
def gather_application_state(session_data, results_data, comparison_data):
    ...
    """
    # Deep copy results_data to avoid modifying the original
    results_data_copy = results_data.copy() if results_data else {}
    
    # If results_data contains a path to a pickle file, load and embed the actual data
...
def load_state_from_file(file_object):
    ...
    # If results were embedded, restore them to a temporary file
