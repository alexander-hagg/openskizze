...
def compute_fitness(heightmap_3d: np.ndarray, wind_direction: int) -> float:
    # print(f"  [DEBUG-FITNESS] Calculating porosity with original wind direction: {wind_direction}°")
    
    # --- THE CRITICAL FIX IS HERE ---
    # To simulate wind FROM a direction, we rotate the grid so that direction
    # becomes the new 'left' side, as our porosity calc sums along columns.
    # A North wind (0°) must be rotated +90° to make the top become the left.
    # A West wind (270°) must be rotated 0° (as it's already on the left).
    rotation_angle = (wind_direction + 90) % 360
    
    # print(f"  [DEBUG-FITNESS] Rotating grid by {rotation_angle}° to align for porosity calculation.")
...
def calculate_features(heightmap: np.ndarray, buildable_mask: np.ndarray, buildable_area_in_sq_meters: float) -> list:
