...
# Check models
ls models/
# Expected: svgp_60m.pth, unet_60m.pth, *_normalization.json (4 files total)
```
...
# Test SVGP
try:
    evaluator = create_evaluator('svgp', parcel_size=60, models_dir=Path('models'), device='cpu')
...
# Test U-Net
try:
    evaluator = create_evaluator('unet', parcel_size=60, models_dir=Path('models'), device='cpu')
...
# Test Hybrid
try:
    evaluator = create_evaluator('hybrid', parcel_size=60, models_dir=Path('models'), device='cpu')
...
evaluator = create_evaluator(model_type, parcel_size=60, models_dir=Path('models'), device=device)
...
for model_type in ['svgp', 'unet', 'hybrid']:
    ...
    evaluator = create_evaluator(model_type, parcel_size=60, device='cuda')
