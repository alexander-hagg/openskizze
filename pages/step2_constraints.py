#
# pages/step2_constraints.py (with Logging)
#
    ...
    return dbc.Container([
        ...
        dbc.Row([
            dbc.Col([
                ...
                dl.Map(
                    id='map-step2', center=[50.734965, 7.055020], zoom=13,
                    children=[
                        dl.TileLayer(),
                        dl.GeoJSON(id='site-polygon-group-step2'),
                        dl.FeatureGroup([dl.EditControl(id='edit-control-taboo', draw={'polygon': True, 'polyline': False, 'circle': False, 'marker': False, 'circlemarker': False, 'rectangle': True}, edit=True)])
            ...
            dbc.Col([
                ...
                dbc.Card(dbc.Checklist(options=[{'label': v, 'value': k} for k, v in MEASURES_OPTIONS.items()], value=list(MEASURES_OPTIONS.keys()), id='measures-checklist', switch=True,), body=True),
                html.Hr(),
...
def display_site_polygon(session_data):
    print("\n--- LOG: Step 2 display_site_polygon triggered ---")
    if session_data and session_data.get('site_polygon'):
        print("LOG: Found site_polygon in session data. Displaying on map.")
        return dl.GeoJSON(data=session_data['site_polygon'], style={'color': 'blue', 'fillOpacity': 0.2})
    
    print("LOG: No site_polygon found in session data for Step 2.")
    return None
